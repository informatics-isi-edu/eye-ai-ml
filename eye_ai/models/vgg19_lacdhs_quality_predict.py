...
def evaluate_model(model_path, test_path, output_path, best_params):
    ...
    with open(Path(output_path) / 'quality_predictions.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Filename', 'True Label', 'Prediction', 'Probability Scores'])
        for i in range(len(filenames)):
            writer.writerow([filenames[i], class_names[y_true[i]], 
                             class_names[y_pred_classes[i]], y_pred[i]])
